...
export default function StaticIPs({ show, selection, setSelection }: { show: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 rounded-md gap-4">
      {plans.map((plan, i) => (
        <div
          key={plan.num}
        ...
        >
          ...
          >
            <div>
              <span className="font-semibold text-xl mr-4">{plan.num}</span>{" "}{plan.num === 1 ? t("stepper.staticIP") : t("stepper.staticIPs")}
            </div>
