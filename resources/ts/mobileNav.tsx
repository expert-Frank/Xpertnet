...
export default function MobileNav() {
    ...
    return (
        <>
            ...
            <aside className={`left-0 transition-transform absolute top-16 w-full z-30 duration-250 bg-white dark:bg-neutral-900 min-h-[calc(100vh-64px)] p-6 flex flex-col justify-between ${opened ? "translate-x-0" : "-translate-x-full"}`}>
                <ul className="flex flex-col gap-4 mt-16">
                    {navItems.map((item, i) => (
                        <li key={i} className="text-3xl font-bold">
                            <button className="cursor-pointer" onClick={() => click(item.anchor)}>{item.label}</button>
                        </li>
                    ))}
                </ul>
                {imprint.source && (
                    <ul class="flex flex-col gap-3 text-sm">
                        <li>
                            <p class="flex gap-2 items-center">
                                ...
                                <span>{imprint['copyright']['year']}, {imprint['copyright']['author']}, {imprint['copyright']['place']}</span>
                            </p>
                        </li>
                        <li class="flex items-center gap-2">
                            {imprint['author']['label']
                                ...
                                .split(';')
                                .flatMap(p => p === ''
                                    ? <a href={imprint['author']['url']} target="_blank" class="underline whitespace-nowrap">{imprint['author']['name']}</a>
                                    : p.split('!').map(r =>
                                        r === '' ? <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="currentColor"  class="text-red-600 icon icon-tabler icons-tabler-filled icon-tabler-heart"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M6.979 3.074a6 6 0 0 1 4.988 1.425l.037 .033l.034 -.03a6 6 0 0 1 4.733 -1.44l.246 .036a6 6 0 0 1 3.364 10.008l-.18 .185l-.048 .041l-7.45 7.379a1 1 0 0 1 -1.313 .082l-.094 -.082l-7.493 -7.422a6 6 0 0 1 3.176 -10.215z" /></svg> : r
                                    )
                                )
                            }
                        </li>
                        <li>
                            <a href={imprint['source']['url']} target="blank" className="flex gap-4 items-center underline">
                                <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeWinecap="round"  strokeLinejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-code"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 8l-4 4l4 4" /><path d="M17 8l4 4l-4 4" /><path d="M14 4l-4 16" /></svg>
